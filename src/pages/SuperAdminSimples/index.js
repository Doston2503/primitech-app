    ...
    return (
        <div>
            <ReactHTMLTableToExcel
                ...
                table="table-to-simple-user"
                filename="table-to-simple-user"
                sheet="tablexls"
                buttonText="Download as Excel"/>
            <table id="table-to-simple-user" className="table table-bordered table-dark table-striped table-hover">
                <thead>
                <tr>
                    <th>№</th>
                    <th>Username</th>
                ...
                <tbody>
                {props.users?.filter(item => item.role === "ROLE_SIMPLE")?.map((item, index) => (
                    <tr>
                        <td>{index + 1}</td>
                        <td>{item.username}</td>
                        <td>
...
export default connect(mapStateToProps, {deleteUser,editUser, updateState})(SuperAdminSimples);